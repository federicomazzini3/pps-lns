...
class DoorTest extends AnyFreeSpec {
  "A collection of doors" - {
    "when empty" - {
      val doors: Map[DoorPosition, DoorState] = Map()
    ...
    "when non empty" - {
      val doors2: Map[DoorPosition, DoorState] = Door(Left -> Close)
      "should allow to add a door" in {
      ...
      "should allow to close all door" in {
        val doors: Map[DoorPosition, DoorState] = (Left -> Open) :+ (Right -> Open) :+ (Above -> Lock)
        assert(
          doors.close
            .filter((_, state) =>
              state match {
