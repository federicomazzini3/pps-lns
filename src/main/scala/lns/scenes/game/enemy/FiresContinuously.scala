...
trait FiresContinuously { this: EnemyModel with FireModel =>
  def computeFire(context: FrameContext[StartupData])(character: AnythingModel): Option[Vector2] = status.head match {
    case (EnemyState.Attacking, _) => Some((character.getPosition() - getPosition()).normalise)
    case _                         => None
  }
